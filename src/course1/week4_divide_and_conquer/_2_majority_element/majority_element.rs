...
pub fn get_majority_nlogn(a: &[u32]) -> Option<u32> {
    let a_len = a.len();
    if a_len == 0 {
        return None;
    }
    ...
    let (major, el) = if l_count > r_count {
        (l_count, left)
    } else {
        (r_count, right)
    };
...
pub fn get_majority_fast(a: &[u32]) -> Option<u32> {
    if a.len() == 0 {
        return None;
    }
...
pub fn get_majority_naive(a: &[u32]) -> Option<u32> {
    for i in a {
        let mut count = 0;
        for j in a {
            if j == i {
